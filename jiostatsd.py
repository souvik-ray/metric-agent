...
class StatsdClient(object):
    ...
    def sample(data, sample_rate):
        ...
        >>> StatsdClient.daemon({"example.sample2": "2"}, 1)
        {'example.sample2': '2'}
        >>> StatsdClient.daemon({"example.sample3": "3"}, 0)
        {}
        >>> from random import seed
        >>> seed(1)
        >>> sampled = StatsdClient.daemon({"example.sample5": "5", "example.sample7": "7"}, 0.99)
        >>> len(sampled)
        2
        ...
        '7|@0.99'
        >>> StatsdClient.daemon({"example.sample5": "5", "example.sample7": "7"}, 0.01)
        {}
        """
        if sample_rate >= 1:
            return data
